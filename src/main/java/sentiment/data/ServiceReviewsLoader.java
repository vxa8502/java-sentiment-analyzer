...
class ServiceReviewsLoader extends CsvLoaderBase {
    ...
    protected Dataset processRecord(CSVRecord record, DatasetLoadingStats stats, List<String> headers) {
        ...
        if (record.size() >= 6 && (headers.isEmpty() || !record.isMapped("text"))) {
            ...
            // Use centralized FieldExtractor mappings for flexible field extraction
        ...
        // Use base class helper to validate and build
        Dataset dataset = validateAndBuild(reviewText, sentimentLabel, recordNumber, stats, "service", DATASET_TYPE);

        // Add review/ticket ID if available
        if (dataset != null && reviewId != null && !reviewId.trim().isEmpty()) {
            // Rebuild with ID
            return new Dataset.Builder(dataset.getText(), dataset.getSentiment())
                    .source(dataset.getSource())
                    .originalLabel(dataset.getOriginalLabel())
