    ...
    public SVMConfig findOptimalConfig(Instances trainingData, boolean enableClassWeighting) throws Exception {
        ...
        for (SVMConfig config : configGrid) {
            ...
            try {
                ...
                logger.info("  -> Macro-F1: {:.4f} (±{:.4f}), Accuracy: {:.4f}",
                        config.getCvMacroF1(), config.getCvStdDev(), config.getCvAccuracy());
            } catch (Exception e) {
        ...
        logger.info("  -> Macro-F1: {:.4f} (±{:.4f})", bestConfig.getCvMacroF1(), bestConfig.getCvStdDev());
        logger.info("  -> Accuracy: {:.4f}", bestConfig.getCvAccuracy());
