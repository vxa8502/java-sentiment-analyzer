...
public class WekaModelPersistence<T extends ClassifierTrainingTemplate<?>> {
    ...
    private Classifier extractWekaClassifier(T classifier) {
        if (classifier instanceof NaiveBayesClassifier nb) {
            return nb.getNaiveBayes();
        } else if (classifier instanceof RandomForestClassifier rf) {
            return rf.getRandomForest();
        } else if (classifier instanceof LogisticRegressionClassifier lr) {
            return lr.getLogistic();
        } else if (classifier instanceof SVMClassifier svm) {
            return svm.getSMO();
        } else {
            throw new UnsupportedOperationException(
                    "Unsupported classifier type: " + classifier.getClass().getName());
        }
    }

    private Instances extractTrainingStructure(T classifier) {
        if (classifier instanceof NaiveBayesClassifier nb) {
            return nb.getTrainingStructure();
        } else if (classifier instanceof RandomForestClassifier rf) {
            return rf.getTrainingStructure();
        } else if (classifier instanceof LogisticRegressionClassifier lr) {
            return lr.getTrainingStructure();
        } else if (classifier instanceof SVMClassifier svm) {
            return svm.getTrainingStructure();
        } else {
            throw new UnsupportedOperationException(
                    "Unsupported classifier type: " + classifier.getClass().getName());
        }
    ...
    private void setWekaClassifier(T classifier, Classifier wekaClassifier) {
        if (classifier instanceof NaiveBayesClassifier nb) {
            nb.setNaiveBayes((weka.classifiers.bayes.NaiveBayes) wekaClassifier);
        } else if (classifier instanceof RandomForestClassifier rf) {
            rf.setRandomForest((weka.classifiers.trees.RandomForest) wekaClassifier);
        } else if (classifier instanceof LogisticRegressionClassifier lr) {
            lr.setLogistic((weka.classifiers.functions.Logistic) wekaClassifier);
        } else if (classifier instanceof SVMClassifier svm) {
            svm.setSMO((weka.classifiers.functions.SMO) wekaClassifier);
        } else {
            throw new UnsupportedOperationException(
                    "Unsupported classifier type: " + classifier.getClass().getName());
        }
    ...
    private void setTrainingMetadata(T classifier, Instances structure, String[] classes) {
        if (classifier instanceof NaiveBayesClassifier nb) {
            nb.setTrainingMetadata(structure, classes);
        } else if (classifier instanceof RandomForestClassifier rf) {
            rf.setTrainingMetadata(structure, classes);
        } else if (classifier instanceof LogisticRegressionClassifier lr) {
            lr.setTrainingMetadata(structure, classes);
        } else if (classifier instanceof SVMClassifier svm) {
            svm.setTrainingMetadata(structure, classes);
        } else {
            throw new UnsupportedOperationException(
                    "Unsupported classifier type: " + classifier.getClass().getName());
        }
